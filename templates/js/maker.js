...
skillsAutoCompleteList = ["Python", "Java", "JavaScript", "C++", "C#", "React.js", "Node.js", "HTML", "CSS", "SQL", "MongoDB", "AWS", "Azure", "Docker", "Kubernetes", "Machine Learning", "Data Science", "Artificial Intelligence", "Deep Learning", "TensorFlow", "PyTorch", "Data Analysis", "Big Data", "Cybersecurity", "Blockchain", "DevOps", "Git", "REST APIs", "Agile", "Scrum", "Cloud Computing", "SEO", "Digital Marketing", "Google Analytics", "Google Ads", "PPC", "Content Writing", "Copywriting", "Social Media Marketing", "Email Marketing", "E-commerce", "UI/UX Design", "Figma", "Adobe Photoshop", "Adobe Illustrator", "Adobe XD", "Wireframing", "Prototyping", "Graphic Design", "Video Editing", "Animation", "3D Modeling", "AutoCAD", "SolidWorks", "MATLAB", "SQL Server", "Power BI", "Tableau", "Excel", "Microsoft Office", "Project Management", "Leadership", "Team Management", "Time Management", "Problem-Solving", "Critical Thinking", "Communication", "Customer Service", "Salesforce", "CRM", "Financial Analysis", "Budgeting", "Accounting", "Risk Management", "Event Planning", "Teaching", "Research", "Medical Coding", "Product Management", "Quality Assurance", "Testing", "Embedded Systems", "Robotics", "IoT", "Swift", "Kotlin", "Flutter", "Django", "Laravel", "Vue.js", "Swift", "Ruby on Rails"];
let skillsAutoComplete = new Awesomplete(document.getElementById("skills"), {
  list: skillsAutoCompleteList ,
  minChars:1,
});
...
skillsinput.addEventListener("input", function (e){
  ...
  if(inputValue.includes(",")){
    ...
    // if(lastskill.length>0){
      new Awesomplete(document.getElementById("skills"), {
        list: skillsAutoCompleteList ,
        minChars:1,
      });
...
skillsinput.addEventListener("keydown",function(e){
  if (e.key=="Enter"){
    ...
    if(!val.endsWith(",")){
      let up = document.getElementById("skillsEntered")
      up.innerHTML = `${up.innerHTML} <div>${val}</div>` 

      document.getElementById("skills1").value += `${val},`;
      e.target.value=''
...
let enteredSkills = document.querySelectorAll("#skillsEntered div");

document.getElementById("skillsEntered").addEventListener("click", function (e) {
  if (e.target.tagName.toLowerCase() === 'div') {
    let deletedValue = e.target.innerHTML.trim();;

    // Get all the skills in the input field
    let allValues = document.getElementById("skills1").value;
    // Remove the deleted value from the string
    let updatedValues = allValues.split(',').filter(skill => skill.trim() !== deletedValue).join(',');
    // Update the input field with the new value
    document.getElementById("skills1").value = updatedValues;

    let checker = document.querySelector("#skillsEntered div");
    if(checker){
      e.target.remove();
    }
  }
